...
echo "Generating bundles for all contracts..."

# We pass all contracts in a SINGLE command to ensure they all end up in the same MANIFEST.txt
./generate_hedera_sc_metadata.sh \\
  IdentityRegistryUpgradeable=${identityImpl.address} \\
  ReputationRegistryUpgradeable=${reputationImpl.address} \\
  ValidationRegistryUpgradeable=${validationImpl.address} \\
  ERC1967Proxy=${identityProxy.address}
